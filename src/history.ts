...
function sourceTurnEvents(snapshot: SessionLogSnapshot, source: HistoryTurnSource): readonly SessionEvent[] {
  const end = snapshot.events[source.sourceBoundarySeq]
  if (end?.type !== 'turn/end' || end.data.turn !== source.sourceTurn) {
    throw new Error(
      `source session "${source.sourceSessionId}" seq ${source.sourceBoundarySeq} is not turn ${source.sourceTurn}'s end`,
    )
  }
  let startIndex = -1
...
function mappedSources(event: SessionEvent, seqMap: ReadonlyMap<number, number>): number[] | undefined {
  if (!('sourceEventSeqs' in event) || event.sourceEventSeqs === undefined) return undefined
...
export function appendHistoricalTurn(
  target: Session,

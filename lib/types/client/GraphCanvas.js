...
export function GraphCanvas({ state, previewNodeId, onPreview, selectedNodeIds, candidateNodeId, disabled = false, labels: suppliedLabels, nodeColors, fit = true, }) {
    ...
    return _jsx("div", { ref: viewportRef, className: `dsh-git-tree-viewport ${fit ? '' : 'dsh-git-tree-viewport-scroll'}`, children: _jsx("div", { className: "dsh-git-tree-fit", style: { width: fittedWidth, height: fittedHeight }, children: _jsxs("div", { className: "dsh-git-tree-stage", style: { width: layout.width, height: layout.height, transform: `scale(${scale})` }, children: [_jsx("svg", { className: "dsh-git-tree-svg", width: layout.width, height: layout.height, "aria-hidden": "true", children: orderedNodes(state).flatMap(node => node.parentIds.map(parentId => {

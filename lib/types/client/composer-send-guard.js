 ...
 */
export function installComposerSendGuard(anchor, onRefused) {
    ...
    const onKeyDown = (event) => {
        ...
        if (event.isComposing || menuOpen(card))
            return;
        refuse(event);
    };
    const onPointer = (event) => {
        ...
        if (button === null || button !== sendButton(card))
            return;
        refuse(event);
    };
...
//# sourceMappingURL=composer-send-guard.js.map
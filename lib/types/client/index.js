            ...
            return {
                ...
                ask: async (question, manifest) => {
                    ...
                    if (selected.length === 0)
                        throw new Error('请先选择至少一个 PA 节点。');
                    ...
                    if (source === undefined)
                        throw new Error('无法访问用于创建 merge branch 的来源 session。');
                    ...
                    if (!command.ok)
                        throw new Error(`创建 merge branch 失败：${command.error.message}`);
                    if (!command.value.matched)
                        throw new Error('Host 未加载 dsh-git 历史合成命令，请重启 dsh。');

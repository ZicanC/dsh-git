import { CREATE_MERGED_SESSION_RPC_ENDPOINT, HISTORY_PREVIEW_RPC_ENDPOINT, PROJECT_GRAPH_RPC_CHANNEL, PROJECT_GRAPH_RPC_ENDPOINT, decodeCreateMergedSessionResponse, decodeHistoryPreviewResponse, decodeProjectGraphResponse, } from "../protocol.js";
import { ComposerDiscardAction } from "./ComposerDiscardAction.js";
import { ContextTrayDock } from "./ContextTrayDock.js";
import { GraphView } from "./GraphView.js";
import { ComposerBlockLease } from "./composer-block-lease.js";
import { ContextTrayChannels } from "./context-tray-channel.js";
import { connectionGraphTransport } from "./graph-transport.js";
    ...
    ctx.slots.inject('conversation.view', () => ctx.slots.register({
        ...
        inject: (sessionId) => {
            ...
            const workspace = () => ctx.workspaces.list.getSnapshot().items
                .find(candidate => candidate.sessionIds.some((id) => String(id) === String(sessionId)));
            ...
            return {
                ...
                loadPreviewImage: async (sourceSessionId, attachment) => {
                    ...
                    if (binding === undefined)
                        throw new Error(`Cannot access preview image Session ${sourceSessionId}`);
                ...
                createMergedSession: async (manifest, draft, signal) => {
                    ...
                    if (draft.hasStructuredReferences) {
                        throw new Error(localized('输入草稿包含无法跨 Session 转移的结构化引用。', 'The draft contains structured references that cannot move between Sessions.'));
                    }
                    ...
                    if (response.targetSessionId !== childSessionId) {
                        throw new Error(localized('Host 返回了错误的新 Chat ID。', 'The Host returned the wrong new Chat ID.'));
                    }
                    signal.throwIfAborted();
                    ...
                    if (childBinding === undefined) {
                        throw new Error(localized(`新 Chat ${childSessionId} 已创建，但浏览器尚未收到对应 Session；原草稿未改动。`, `The new Chat ${childSessionId} was created, but its Session has not reached the browser; the source draft was preserved.`));
                    }
                    ...
                    if (sourceDraft.phase !== 'plain'
                        ...
                        throw new Error(localized(`新 Chat ${childSessionId} 已创建，但来源草稿在创建期间发生变化；为避免覆盖，新旧草稿均未转移或清除。`, `The new Chat ${childSessionId} was created, but the source draft changed during creation. Neither draft was moved or cleared, to avoid overwriting newer input.`));
                    }
                    signal.throwIfAborted();
                    if (draft.imageIds.length > 0 && !childInput.addImages(draft.imageIds)) {
                        throw new Error(localized(`新 Chat ${childSessionId} 已创建，但图片草稿无法转移；原草稿未改动。`, `The new Chat ${childSessionId} was created, but its image draft could not be transferred; the source draft was preserved.`));
                    }
                    childInput.setDraft(draft.text);
                    sourceInput.setDraft('');
                    for (const imageId of draft.imageIds)
                        sourceInput.removeImage(imageId);
                    signal.throwIfAborted();
                    sessions.open(childSessionId);
                    if (!await activateBranchesTab(sessions, childSessionId)) {
                        childInput.notify('error', localized('新 Chat 已创建并打开，但无法自动切回 Branches；请手动点击“分支”。', 'The new Chat was created and opened, but Branches could not be activated automatically; click “Branches” manually.'));
                    }
                },
            };
        },

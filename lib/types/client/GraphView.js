...
export function GraphView({ sessionId, useSession, useInput, inputActions, useGraph, syncTurns, adoptObservedGraph, loadProjectGraph, loadHistoryPreview, loadPreviewImage, setComposerBlocked, createMergedSession, }) {
    ...
    const move = (nodeId, beforeId) => {
        if (busy)
            return;
        setSelectedIds(ids => {
            if (nodeId === beforeId || !ids.includes(nodeId))
                return ids;
    ...
    return _jsxs("div", { className: "dsh-git-root", "data-conversation-composer-overlay": "", children: [_jsxs("div", { className: "dsh-git-workbench", children: [_jsxs("div", { className: `dsh-git-branch-left ${inspected === undefined ? '' : 'dsh-git-branch-left-open'}`, children: [_jsxs("section", { className: "dsh-git-graph-panel", "aria-label": "Conversation Graph", children: [_jsxs("header", { className: "dsh-git-heading", children: [_jsx("span", { children: "Conversation Graph" }), _jsx("span", { className: "dsh-git-muted", children: projectError ?? localized('蓝色：已加入 · 绿色：预览', 'Blue: included · green: preview', locale) })] }), _jsx(GraphCanvas, { state: canvasState, previewNodeId: inspectedId, selectedNodeIds: selectedIds, candidateNodeId: candidateId, disabled: busy, onPreview: inspect })] }), inspected === undefined || inspectedId === null ? null : _jsx(PAContextWindow, { state: state, nodeId: inspectedId, label: labels.get(inspectedId) ?? 'PA', selected: inspectedSelected, disabled: busy, onAdd: addCandidate, onRemove: () => remove(inspectedId), onClose: closeInspector })] }), _jsxs("section", { className: "dsh-git-chat-panel", "aria-label": "Chat History", children: [_jsxs("header", { className: "dsh-git-heading", children: [_jsx("span", { children: "Chat History" }), _jsxs("span", { className: "dsh-git-muted", children: [selectedIds.length, " ", localized('已加入', 'included', locale), candidateId === null ? '' : localized(' + 1 预览', ' + 1 preview', locale)] })] }), _jsx(ChatHistoryPreview, { response: preview, orderedNodeIds: orderedPreviewIds, labels: labels, candidateNodeId: candidateId, loading: previewLoading, error: previewError, loadImage: loadPreviewImage })] })] }), _jsx(ContextTray, { state: state, selectedIds: selectedIds, candidateId: candidateId, busy: busy, error: actionError, dirty: dirty, draftHasContent: draftHasContent, overLimit: selectedIds.length > MAX_MERGED_HISTORY_TURNS, onMove: move, onMoveEnd: moveEnd, onRemove: remove, onClear: () => { setSelectedIds([]); setCandidateId(null); setInspectedId(null); setSelectionTouched(true); }, onMerge: merge, onDiscard: discard })] });
}
